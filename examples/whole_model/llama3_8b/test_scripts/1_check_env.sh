...
fmha_dir="$FACT_ROOT/pattern_table/fmha/fp16/sm80/fmha_llama3_gqa"
swiglu_dir="$FACT_ROOT/pattern_table/gemm/fp16/sm80/swiglu_mlp_fusion"
...
if [ ! -d "$swiglu_dir" ]; then
    echo "✗ ERROR: SwiGLU pattern directory not found: $swiglu_dir"
    exit 1
else
    echo "✓ SwiGLU pattern directory found"
fi
...
required_files=(
    ...
    "$swiglu_dir/Model.py"
    "$swiglu_dir/ModelNew.py"
    "$swiglu_dir/cutlass_kernels/swiglu_mlp_fusion.cu"
    "$swiglu_dir/cutlass_kernels/main.cpp"
    "$WHOLE_MODEL/ModelNew.py"
    "$WHOLE_MODEL/test.py"
)
