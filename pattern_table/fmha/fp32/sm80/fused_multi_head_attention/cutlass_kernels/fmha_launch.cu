 ...
 *
 * External API: FP32 tensors
 * Internal computation: FP16 (CUTLASS FMHA requires half precision)
 **************************************************************************************************/
...
// Tile configuration for head_dim_v > 64 (our case: head_dim_v = 96)
...
constexpr int kMaxK = 128;              // Max head_dim_v we support (up to 128)
...
torch::Tensor fmha_forward_cuda(
    ...
    // Causal masking (from top-left)
    params.custom_mask_type = FMHAKernel::CausalFromTopLeft;
